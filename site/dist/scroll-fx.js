// Auto-kompiliert aus site/scroll-fx.jsx (kein Browser-Babel mehr).
// Quelle bleibt site/scroll-fx.jsx — bei Änderungen neu kompilieren.
(function () {
...
function fxEnsureBound() {
  if (fxBound) return;
  fxBound = true;
  window.addEventListener('scroll', fxOnScroll, {
    passive: true
  });
  window.addEventListener('resize', fxOnScroll, {
    passive: true
  });
}
...
function useReducedMotion() {
  const [reduced, setReduced] = useFXState(() => typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
  useFXEffect(() => {
...
function CountUpFX({
  ...
  const display = decimals > 0 ? Number(val).toFixed(decimals) : Math.round(val).toLocaleString('de-CH');
